...
karatsuba a b c n1 n2 = do
  ...
  cond
    ...
    \case
      ...
      False -> do
        x <- a `locally` \unwrap -> do return $ f (unwrap n1) (unwrap n2)
        ...
        z1 <- a `locally` \unwrap -> do return $ unwrap z1' - unwrap z2 - unwrap z0
        a `locally` \unwrap -> do return let s = splitter (unwrap x) in (unwrap z2 * s * s) + (unwrap z1 * s) + unwrap z0
        where
...
mainChoreo = do
  n1 <- primary `locally` \_ -> do return 3141592
  n2 <- primary `locally` \_ -> do return 2718281
